...
네오는 평소 프로도가 비상금을 숨겨놓는 장소를 알려줄 비밀지도를 손에 넣었다. 그런데 이 비밀지도는
 숫자로 암호화되어 있어 위치를 확인하기 위해서는 암호를 해독해야 한다. 다행히 지도 암호를
 해독할 방법을 적어놓은 메모도 함께 발견했다.
...
public class Algo11 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < n; i++) {
			binary1[i] = String.format("%0" + n + "d", Long.parseLong(Long.toBinaryString(arr1[i])));
			binary2[i] = String.format("%0" + n + "d", Long.parseLong(Long.toBinaryString(arr2[i])));
		}
